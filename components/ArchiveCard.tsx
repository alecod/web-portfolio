...
const ArchiveCard = ({title, des, listItem, link}: ArchiveCardProps) => {
  return (
    <a href={link}>
        <div className='w-full h-85 rounded-lg bg-[#112240] p-7 flex flex-col justify-center gap-6 hover:-translate-y-2 transition-transform duration-300 group'>
        ...
        <ul className='text-xs mdl:text-sm text-textDark flex items-center gap-2 justify-between flex-wrap'>
           {listItem.map((item: string, i: number) => (
            <li key={i}>{item}</li>
           ))}
        </ul>
    </div>
